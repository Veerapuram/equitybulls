          ...
          <div class="single_page">
            <div class="social_link">
              ...
              <div class="single_post_content">
                <h2><span>Keywords Selected:&nbsp;&nbsp;<?php echo $keyWords;?></span></h2>
              </div>  
              <?php
                    ...
                    $newsCategoryResult = $controller->listCategoryNames();
                    if(!empty($newsCategoryResult))
                    {
                      $categoryNameArray = json_decode( $newsCategoryResult, true );
                      if(!empty($categoryNameArray))
                      {
                        foreach($categoryNameArray as $categoryNameInfo) {
                        $categoryID = $categoryNameInfo['news_category_id'];
                        $categoryName = $categoryNameInfo['news_category_name'];
              ?>
              <div class="col-lg-12 col-md-12 col-sm-12">
                <div class="left_content">
                  <div class="single_post_content">
                      <h2><span><?php echo $categoryName;?></span></h2>
                        <div class="single_post_content_center">
                          <ul class="spost_nav">
                            <?php
                              global $fileName;
                              $newsDetailsStockResult = $controller->listNewsDetailsCategoryKeyWords($categoryID,$keyWords,$startFrom, $showRecordPerPage);
                              if(!empty($newsDetailsStockResult))
                              {
                                ...
                                if(!empty($newsStockArray))
                                {
                                  foreach($newsStockArray as $newsStockInfo) {
                                    ...
                                    echo '<li>';
                                    echo '  <div class="media wow fadeInDown"> <a href="category.php?cat='.$categoryID.'&news='.$newsID.'" class="media-left"> <img alt="" src="images/'.$fileName.'"> </a>';
                                    echo '  <div class="media-body"> <a href="category.php?cat='.$categoryID.'&news='.$newsID.'" class="catg_title">'.$keyword.'</a> </div>';
                                    echo '</div>';
                                    echo '</li>';
                                  }    
                                }
              ...
              <nav aria-label="Page navigation">
                <ul class="pagination justify-content-center">
                  ...
                  <?php if($currentPage >= 2) { ?>
                        <li class="page-item"><a class="page-link" href="?page=<?php echo $previousPage ?>&cat=<?php echo $categoryID?>&news=<?php echo $newsItem;?>&keywords=<?php echo $keyWords;?>"><?php echo $previousPage ?></a></li>
                  <?php } ?>
                      <li class="page-item active"><a class="page-link" href="?page=<?php echo $currentPage; ?>&cat=<?php echo $categoryID?>&news=<?php echo $newsItem;?>&keywords=<?php echo $keyWords;?>"><?php echo $currentPage ?></a></li>
                  <?php if($currentPage != $lastPage) { ?>
                      <li class="page-item"><a class="page-link" href="?page=<?php echo $nextPage ?>&cat=<?php echo $categoryID?>&news=<?php echo $newsItem;?>&keywords=<?php echo $keyWords;?>"><?php echo $nextPage ?></a></li>
                       <li class="page-item">
                          <a class="page-link" href="?page=<?php echo $lastPage ?>&cat=<?php echo $categoryID?>&news=<?php echo $newsItem;?>&keywords=<?php echo $keyWords;?>" aria-label="Next">
