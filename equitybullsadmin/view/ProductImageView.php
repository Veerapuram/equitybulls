<?php	session_start();
    ...
    ini_set('display_errors', 1);
		...
		{	
			if($_POST['submit']==="add")
				$controller->addProductImages($_POST);
			if($_POST['submit']==="update")
				$controller->updProductImages($_POST);
		}	
		else if(isset($_POST['reset']) && ($_POST['reset']==="delete"))
		{
			$controller->delProductImages($_POST);
		}

		$companyResult  = $controller->listCompany();
		$productImagesResult = $controller->listProductImages();
?>
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <title>Equity Bulls | Product Image Details </title>
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1>Product Images Entry Form</h1>
          </div>
          <div class="col-sm-6">
          ...
          <div class="col-md-6">
            ...
            <div class="card card-primary">
              <div class="card-header">
                <h3 class="card-title">Entry form for Product Images</h3>
              </div>
              <!-- /.card-header -->
              <!-- form start -->
              <form name="productImages" method="post" enctype="multipart/form-data">
                <div class="card-body">
					<div class="form-group">
						<label for="company">Company Name</label>
						<select id="company" name="company" class="form-control select2 required" style="width: 100%;">
						<?php
					...
					<div class="form-group">
						<label for="productsImage">Products for Company</label>
						<input type="file" class="form-control form-control-border" id="productsImage[]" name="productsImage[]" multiple="multiple" />
					</div>
					<div class="form-group">
						<label for="productsImage">Product Selected for Company</label>
      ...
      <div class="container-fluid">
        <div class="row">
          ...
          <div class="col-md-12">
            ...
            <div class="card card-primary">
              <div class="card-header">
                <h3 class="card-title">List of Product Images</h3>
              </div>
				<table id="productImagesList" name="productImagesList" class="display table table-bordered table-hover" style="width:100%">
					<thead>
						<tr>
							<th>File ID</th>
							<th>Company ID</th>
					...
					<tbody>
						<?php
							if(!empty($productImagesResult))
							{
								$productImagesArray = json_decode( $productImagesResult, true );
								if(!empty($productImagesArray))
								{
									foreach($productImagesArray as $productImagesInfo) {
										$fileID             = $productImagesInfo['file_id'];
										$companyISIN	 		= $productImagesInfo['company_isin'];
										$companyName		= $productImagesInfo['company_name'];
										$fileDetails		= $productImagesInfo['file_details'];
										$status				= $productImagesInfo['file_status'];
...
$(document).ready(function() {
    var table = $('#productImagesList').DataTable({
		"columnDefs": [
            {
                "targets": [ 0 ],
                "visible": false,
	...
	$('#productImagesList tbody').on('click', 'tr', function () {
    var data = table.row( this ).data();
		var fileID        = data[0]
		var companyISIN     = data[1];
		var productStatus = data[4];
		
		$("#fileID").val(fileID);
		$("#fileID").val();
			
		$("#company").val(companyISIN).trigger("change");

		$("#status").select2().select2('val',productStatus)
		
		$("#dispImgName").val(data[3]);
