<?php	ini_set('display_errors', 1);
		...
		include_once("../controller/IndustryController.php");
		$controller = new Controller();
		if(isset($_POST['submit']))
		{	
			if($_POST['submit']==="add")
				$controller->addIndustry($_POST);
			if($_POST['submit']==="update")
				$controller->updIndustry($_POST);
		}	
		else if(isset($_POST['reset']) && ($_POST['reset']==="delete"))
		{
			$controller->delIndustry($_POST);
		}
        ...
        <div class="row">
          ...
          <div class="col-md-6">
            ...
            <div class="card card-primary">
              ...
              <!-- /.card-header -->
              <!-- form start -->
              <form name="industryMaster" method="post">
                <div class="card-body">
                   <div class="form-group">
						<label for="industryName">Industry Name<span class="required">*</span></label>
						<input type="text" class="form-control form-control-border" required="required" id="industryName" name="industryName" placeholder="Enter Industry Name">
                  </div>
  				   <div class="form-group">
						<label>Status</label>
      ...
      <div class="container-fluid">
        <div class="row">
          ...
          <div class="col-md-12">
            ...
            <div class="card card-primary">
              <div class="card-header">
                <h3 class="card-title">List of Industry Entries</h3>
              </div>
				<table id="industryDetails" name="industryDetails" class="display table table-bordered table-hover" style="width:100%">
					<thead>
						<tr>
							<th>ID</th>
							<th>Industry Name</th>
							...
							if(!empty($industryResult))
							{
								...
								if(!empty($industryArray))
								{
									foreach($industryArray as $industryInfo) {
										...
										$industryStatus 	= $industryInfo['industry_status'];
										$industryStatus		= ($industryStatus==="A")?"Active":"Inactive";
										echo "<tr>
												<td>".$industryID."</td>
												<td>".$industryName."</td>
												<td>".$industryStatus."</td>
											</tr>";
									}
								}
							}
...
$(document).ready(function() {
    var table = $('#industryDetails').DataTable({
		"columnDefs": [
            {
                "targets": [ 0 ],
                "visible": false,
	...
	$('#industryDetails tbody').on('click', 'tr', function () {
        var data = table.row( this ).data();
		var industryID 		= data[0];
		var industryStatus 	= data[2];
		
		$('#industryName').val(data[1]);
		
		$("#status").select2().select2('val',industryStatus);
